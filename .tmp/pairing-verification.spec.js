...
async function selectOption(page, triggerId, label) {
  await page.locator('#' + triggerId).click();
  await page.getByRole('option', { name: label, exact: true }).click();
}

test('verify custom muscle pairing flows', async ({ page }) => {
  const consoleErrors = [];
  const pageErrors = [];
  ...
  for (const [primary, secondary] of pairings) {
    ...
    try {
      ...
      if (await firstSwapButton.isEnabled()) {
        ...
        const exercisesAfterSwap = await page.locator('h4.font-semibold.text-foreground').allTextContents();
  ...
  expect(results.length).toBe(pairings.length);
});
